...
def create_job(app_id, env_vars, image="pipeflow/cast-sh:dev", namespace="default"):
    ...
    obj_meta = kube.client.V1ObjectMeta(
        labels={"app": "cast-{}".format(app_id)}, name="cast-{}".format(app_id)
    )
    ...
    for key in env_vars.keys():
        ...
        var = kube.client.V1EnvVar(name=key, value=env_vars[key])

        envs.append(var)

    isgx = kube.client.V1VolumeMount(mount_path="/dev/isgx", name="dev-isgx")
...
def create_service(app_id, namespace="default"):
    ...
    svc_meta = {
        "name": "cast-{}".format(app_id),
        "labels": {"app": "cast-{}".format(app_id)},
    }

    svc_spec = {
        "ports": [{"protocol": "TCP", "port": 5000, "targetPort": 5000}],
        "selector": {"app": "cast-{}".format(app_id)},
        "type": "NodePort",
    }
    ...
    s = CoreV1Api.create_namespaced_service(namespace=namespace, body=svc)
...
def get_node_ip():
    ...
    for node in nodes:
        is_ready = [s for s in node.status.conditions if s.type == "Ready"][
            0
        ].status == "True"
        if is_ready:
            node_info = node
